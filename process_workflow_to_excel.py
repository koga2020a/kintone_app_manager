...
def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(description='kintoneアプリのプロセス管理ワークフローをExcelに出力')
    ...
    try:
        # アプリディレクトリのパスを取得
        app_dir = OUTPUT_DIR / "5_新しいアプリ"
        # OUTPUT_DIRの内容を表示
        print(f"OUTPUT_DIRのパス: {OUTPUT_DIR.resolve()}")
        print("OUTPUT_DIR内のディレクトリ一覧:")
        for item in OUTPUT_DIR.iterdir():
            if item.is_dir():
                print(f"  {item.resolve()}")
        if not app_dir.exists():
            logger.error(f"アプリID {args.app_id} のディレクトリが見つかりません:{app_dir}")
            print(f"エラー: アプリID {args.app_id} のディレクトリが見つかりません:{app_dir}")
            sys.exit(1)
        
        # プロセス管理設定ファイルを読み込む
        process_file = app_dir / "json" / f"{args.app_id}_process_management.json"
        if not process_file.exists():
            logger.error(f"プロセス管理設定ファイルが見つかりません: {process_file}")
            print(f"エラー: プロセス管理設定ファイルが見つかりません: {process_file}")
            sys.exit(1)
        
        with open(process_file, 'r', encoding='utf-8') as f:
            process_data = json.load(f)
        
        if not process_data or process_data.get('states') is None or process_data.get('actions') is None:
            print(f"警告: {process_file} のプロセスデータが空または無効です。スキップします。")
            return
        ...
        if not output_file:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = app_dir / f"{args.app_id}_workflow_{timestamp}.xlsx"
        else:
            output_file = Path(output_file)
        
        # Excelファイルの作成
        excel_file = create_workflow_excel(args.app_id, process_data, output_file)
        
        logger.info(f"ワークフロー情報を {excel_file} に出力しました")
        print(f"ワークフロー情報を {excel_file} に出力しました")
