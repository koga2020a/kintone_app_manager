...
def create_directory_structure(appid, app_name):
    """
    アプリケーション用のディレクトリ構造を作成し、既存のappid_で始まるディレクトリをバックアップする
...
class ExcelFormatter:
    ...
    def move_cell_str(self, cell_ref, direction):
        """
        指定されたセル参照から、direction に応じて隣接するセルの参照を返す。
        direction: "right", "left", "up", "down" のいずれか
        """
...
def create_excel_report(appid, base_dir):
    ...
    worksheet.column_dimensions['BC'].width = 30
    worksheet.column_dimensions['BD'].width = 25
    worksheet.column_dimensions['BE'].width = 50
    worksheet.column_dimensions['BF'].width = 50
...
def download_app_data(appid, api_token, base_dir, js_dir, json_dir, subdomain, username, password):
    ...
    for name, endpoint in endpoints.items():
        ...
        headers = {
            auth_type: api_token if auth_type == "X-Cybozu-API-Token" else None
        }
        data = fetch_data(url, headers)
